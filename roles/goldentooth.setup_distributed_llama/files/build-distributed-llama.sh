...
if [ -f "Makefile" ]; then
    # Replace -march=native and -mtune=native with ARM64 specific flags
    sed -i 's/-march=native/-march=armv8-a/g' Makefile
    sed -i 's/-mtune=native/-mtune=cortex-a72/g' Makefile
    echo "✅ Makefile patched for ARM64 cross-compilation"
else
    echo "⚠️  Makefile not found, proceeding without patching"
fi
